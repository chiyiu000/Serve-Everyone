...
all_data=[]
for i in range(1, 11):
    url=f'https://guba.eastmoney.com/list,300751_{i}.html'
    print(url)
...
all_data=pd.DataFrame(all_data,columns=['阅读数','评论数','标题', '时间'])
#all_data.to_csv(r'.\600598帖子_t.csv')
#使用SnowNLP计算对每个帖子标题的文字评估情绪得分
def senti(text):
    s=SnowNLP(text)
    return s.sentiments
all_data['情绪']=all_data['标题'].apply(senti)
all_data.to_csv(r'D:\加入时间的股吧爬虫数据\300751帖子+情感_t.csv')
